 ...
 * @since 1.0.0
 */
if ( ! class_exists( 'news_box_About' ) ) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class news_box_About {
		/**
		 ...
		 *
		 * @var news_box_About $instance The  TI_About_Page instance.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof news_box_About ) ) {
				self::$instance = new news_box_About;
		...
		public function ti_about_page_render() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				echo '<div class="wrap about-wrap news_box-about-wrap">';
				...
				echo '<a href="'.esc_url('https://wpthemespace.com/product/news-box').'" target="_blank" class="wp-badge epsilon-welcome-logo"></a>';
    ...
    return add_query_arg(
		...
		public function getting_started() {
			...
			?>
			<div class="feature-section news_box-section news_box-section-getting-started three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .news_box-section -->
			<?php
		}
		...
		public function support() {
			...
			?>
			<div class="feature-section news_box-section news_box-section-support three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .news_box-section -->
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					...
					echo '<tr><td style="border-top:0;"></td><td style="border-top:0;"></td><td style="border-top:0;" class="only-pro"><a href="'.esc_url('https://wpthemespace.com/product/news-box').'" target="_blank" >';
					echo esc_html ('News Box Pro Live Preview', 'news-box');
					echo '</a></td></tr>';
        ...
        'activating_string'        => __( 'Activating', 'news-box' )
        )
    );
